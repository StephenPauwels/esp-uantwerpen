...
const entryType = {ADD_EMPLOYEE: 1, REMOVE_EMPLOYEE: 2, TAG: 3, TYPE: 4};
...
function setupModal() {
    ...
    addMenu.children().click(function () {
        ...
        if (selectedValue === "Employee" || selectedValue === "Werknemer") {
            entries.append(createEditEntry(entryType.ADD_EMPLOYEE));
        } else if (selectedValue === "Type") {
            entries.append(createEditEntry(entryType.TYPE));
        } else if (selectedValue === "Tag") {
            entries.append(createEditEntry(entryType.TAG));
        }
    });
    ...
    removeMenu.children().click(function () {
        ...
        let selectedValue = $(this).text();
        if (selectedValue === "Employee" || selectedValue === "Werknemer") {
            entries.append(createEditEntry(entryType.REMOVE_EMPLOYEE));
        } else if (selectedValue === "Type") {
            entries.append(createEditEntry(entryType.TYPE));
        } else if (selectedValue === "Tag") {
            entries.append(createEditEntry(entryType.TAG));
 ...
 * @param {entryType} type the type of the element
...
function createEditEntry(type) {
    ...
    if (type === entryType.ADD_EMPLOYEE) {
        ...
        placeholder = "Name";
    } else if (type === entryType.REMOVE_EMPLOYEE) {
        className = "employee-entry";
    ...
    if (type === entryType.ADD_EMPLOYEE || type === entryType.REMOVE_EMPLOYEE) {
...
function filterProjects() {
    let filtered_projects = allProjects;
 ...
 * @param {array} projects_filter_prev array with projects to be filtered
 * @param {string} promotor_text
 * @return {array} filtered projects array
 */
function filter_promotor(projects_filter_prev, promotor_text) {
    var filtered_projects = [];
    var project_added;
    for (var i = 0; i < projects_filter_prev.length; i++) {
        project_added = false;
